...
module.exports = class ProgramOperations {
  ...
  async listByUser(req) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let entityAssessorDocumentByUser = await database.models.entityAssessors
          .find({ userId: req.userDetails.id, role: userRole }, { solutionId: 1 })
          .lean();
    ...
    return new Promise(async (resolve, reject) => {
      try {
        let solutionDocument = await database.models.solutions
          .findOne({ _id: ObjectId(req.params._id) }, { entities: 1 })
          .lean();

        let entityTypeDocument = await database.models.entityTypes.findOne({ name: 'school' }, { types: 1 }).lean();

        let administrationTypes = await database.models.entities
          .distinct('metaInformation.administration', { _id: { $in: solutionDocument.entities } })
          .lean();

        administrationTypes = _.compact(administrationTypes)
          .sort()
          .map((administrationType) => {
            return {
  ...
  async userProfile(req) {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let programDocument = await database.models.solutions
          .findOne({ _id: ObjectId(req.params._id) }, { programName: 1 })
          .lean();
  ...
  async entitySummary(req) {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let entityObjects = await opsHelper.getEntities(
          req.userDetails,
          req.query,
          req.pageSize,
          req.pageNo,
        ...
        let entitiesCompletedCount = database.models.submissions
          .countDocuments({ entityId: { $in: entityIds }, status: 'completed' })
          .lean()
          .exec();

        let entitiesInprogressCount = database.models.submissions
          .countDocuments({ entityId: { $in: entityIds }, status: 'inprogress' })
          .lean()
          .exec();

        [entitiesCompletedCount, entitiesInprogressCount] = await Promise.all([
          entitiesCompletedCount,
          entitiesInprogressCount,
        ]);
  ...
  async assessorReport(req) {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        let programDocument = await solutionHelper.solutionDocument(ObjectId(req.params._id), [
          '_id',
          'entities',
          'programId',
          'programName',
        ...
        assessorDetails = database.models.entityAssessors
          .find(assessorQueryObject, { userId: 1, name: 1, entities: 1 })
          .limit(limitValue)
          .skip(skipValue)
          .lean()
          .exec();
        ...
        assessorEntityIds = await opsHelper.getEntities(
          req.userDetails,
          req.query,
          req.pageSize,
          req.pageNo,
        ...
        assessorEntityIds =
          assessorEntityIds.result && assessorEntityIds.result.length
            ? assessorEntityIds.result.map((entity) => entity.id)
            : [];

        let submissionDocuments = await database.models.submissions
          .find({ entityId: { $in: assessorEntityIds } }, { status: 1, createdAt: 1, completedDate: 1, entityId: 1 })
          .lean();
        ...
        assessorDetails.forEach(async (assessor) => {
          let entityByAssessor = opsHelper.getSubmissionByAssessor(
            assessor.userId,
            entitySubmissionMap,
            assessorEntityMap,
          );
        ...
        if (req.query.csv && req.query.csv == 'true') {
          ...
          let result = await opsHelper.constructResultObject(
            'programOperationAssessorReports',
            assessorsReports,
            totalCount,
            req.userDetails,
            programDocument.name,
            req.query,
          );
  ...
  async entityReport(req) {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let entityDocuments = await opsHelper.getEntities(
          req.userDetails,
          req.query,
          req.pageSize,
          req.pageNo,
          !isCSV || isCSV == 'false' ? true : false,
          [],
          req.params._id,
        );
        ...
        entityObjects.forEach(async (singleEntityDocument) => {
          ...
          let resultObject = {};
          resultObject.status = submissionDetails
            ? submissionHelper.mapSubmissionStatus(submissionDetails.status) || submissionDetails.status
            : '';
          resultObject.name = singleEntityDocument.name || '';
          resultObject.daysElapsed = submissionDetails
            ? moment().diff(moment(submissionDetails.createdAt), 'days')
            : '';
          resultObject.assessmentCompletionPercent = submissionDetails
            ? opsHelper.getAssessmentCompletionPercentage(submissionDetails.evidencesStatus)
            : '';
        ...
        if (isCSV == 'true') {
          ...
          result = await opsHelper.constructResultObject(
            'programOperationEntityReports',
            result.entitiesReport,
            totalCount,
            req.userDetails,
  ...
  async searchEntity(req) {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let entityIdAndName = await database.models.entities
          .find(
            {
          ...
          .limit(5)
          .lean(); //autocomplete needs only 5 dataset
