...
module.exports = class Submission extends Abstract {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (Object.keys(appInformation).length > 0) {
          await submissionsHelper.addAppInformation(req.params._id, appInformation, 'submissions');
        }
  ...
  async completeParentInterview(req) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let submissionDocument = await database.models.submissions.findOne(queryObject);
        ...
        let evidencesStatusToBeChanged = submissionDocument.evidencesStatus.find(
          (singleEvidenceStatus) => singleEvidenceStatus.externalId == parentInterviewEvidenceMethod,
        );
            ...
            if (parentInterviewResponse[1].status === 'completed') {
              Object.entries(parentInterviewResponse[1].answers).forEach((answer) => {
                if (evidenceSubmissionAnswerArray[answer[0]]) {
                  ...
                  evidenceSubmissionAnswerArray[answer[0]].countOfInstances =
                    evidenceSubmissionAnswerArray[answer[0]].value.length;
                } else {
        ...
        if (runUpdateQuery) {
          ...
          let canRatingsBeEnabled =
            await submissionsHelper.canEnableRatingQuestionsOfSubmission(updatedSubmissionDocument);
      ...
      try {
        ...
        if (req.body.answers) {
          ...
          Object.entries(req.body.answers).forEach((answer) => {
            if (
              answer[1].isAGeneralQuestion == true &&
              answer[1].responseType === 'matrix' &&
              answer[1].evidenceMethod != ''
            ) {
              runUpdateQuery = true;
              ...
              if (submissionDocument.evidences[answer[1].evidenceMethod].isSubmitted === true) {
                submissionDocument.evidences[answer[1].evidenceMethod].submissions.forEach(
                  (evidenceMethodSubmission, indexOfEvidenceMethodSubmission) => {
                    if (
                      evidenceMethodSubmission.answers[answer[0]] &&
                      evidenceMethodSubmission.answers[answer[0]].notApplicable != true
                    ) {
                      ...
                      answer[1].payload.labels[0].forEach((incomingGeneralQuestionInstancePayload) => {
                        evidenceMethodSubmission.answers[answer[0]].payload.labels[0].push(
                          incomingGeneralQuestionInstancePayload,
                        );
                      });
                      evidenceMethodSubmission.answers[answer[0]].countOfInstances =
                        evidenceMethodSubmission.answers[answer[0]].value.length;
                    }
      ...
      try {
        ...
        if (req.body.parentId && req.body.status && submissionDocument) {
          let parentInformation = await database.models.entities
            .findOne({ _id: ObjectId(req.body.parentId) }, { metaInformation: 1 })
            .lean();
          ...
          if (parentInformation) {
            ...
            if (req.body.status == 'completed') {
              ...
              parentInterview.startedAt =
                !submissionDocument.parentInterviewResponses ||
                !submissionDocument.parentInterviewResponses[req.body.parentId] ||
                !submissionDocument.parentInterviewResponses[req.body.parentId].startedAt
                  ? new Date()
                  : submissionDocument.parentInterviewResponses[req.body.parentId].startedAt;
            ...
            if (submissionDocument.parentInterviewResponses) {
              submissionDocument.parentInterviewResponses[req.body.parentId] = _.merge(
                submissionDocument.parentInterviewResponses[req.body.parentId],
                parentInterview,
              );
            } else {
            ...
            let parentInterviewResponseStatus = _.omit(submissionDocument.parentInterviewResponses[req.body.parentId], [
              'parentInformation',
              'answers',
            ]);
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (req.query.parentId && submissionDocument) {
          ...
          if (
            submissionDocument.parentInterviewResponses &&
            submissionDocument.parentInterviewResponses[req.query.parentId]
          ) {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        for (
          ...
          pointerToResultingArray < resultingArray.length;
          pointerToResultingArray++
        ) {
  ...
  async dummyRate(req) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let submissionDocument = await database.models.submissions.findOne(queryObject, { criteria: 1 }).lean();
        ...
        if (true) {
          let criteriaData = await Promise.all(
            submissionDocument.criteria.map(async (criteria) => {
              if (
                !criteria.score ||
                criteria.score != '' ||
                criteria.score == 'No Level Matched' ||
                criteria.score == 'NA'
              ) {
  ...
  */
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (!submissionDocument || !submissionDocument._id) {
          ...
          if (
            submissionDocument.evidencesStatus[0].isSubmitted &&
            submissionDocument.evidencesStatus[0].isSubmitted == true
          ) {
  ...
  async feedback(req) {
    return new Promise(async (resolve, reject) => {
      ...
      if (runUpdateQuery) {
        let result = await database.models.submissions.findOneAndUpdate(queryObject, updateObject);
      ...
      try {
        ...
        if (ecmData.isSubmitted == true) {
          for (
            ...
            pointerToSubmissions < ecmData.submissions.length;
            ...
            if (currentEcmSubmissions.isValid === true) {
              Object.entries(currentEcmSubmissions.answers).forEach((answer) => {
                if (answer[1].responseType === 'matrix' && answer[1].notApplicable != true) {
                  for (let countOfInstances = 0; countOfInstances < answer[1].value.length; countOfInstances++) {
                    answer[1].value[countOfInstances] &&
                      _.valuesIn(answer[1].value[countOfInstances]).forEach((question) => {
                        if (question.qid && answerArray[question.qid]) {
                          answerArray[question.qid].instanceResponses &&
                            answerArray[question.qid].instanceResponses.push(question.value);
                          answerArray[question.qid].instanceRemarks &&
                            answerArray[question.qid].instanceRemarks.push(question.remarks);
                          answerArray[question.qid].instanceFileName &&
                            answerArray[question.qid].instanceFileName.push(question.fileName);
                        } else {
                          let clonedQuestion = { ...question };
  ...
  async modifyByCsvUpload(req) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let solutionData = await database.models.solutions
          .findOne(
            {
        ...
        let questionDocument = await database.models.questions
          .find(
            {
              _id: { $in: questionIds },
              externalId: { $in: questionCodeIds },
            },
            { _id: 1, externalId: 1, responseType: 1, options: 1 },
          )
          .lean();
        ...
        for (
          let pointerTosubmissionUpdateData = 0;
          pointerTosubmissionUpdateData < chunkOfsubmissionUpdateData.length;
          pointerTosubmissionUpdateData++
        ) {
          await Promise.all(
            chunkOfsubmissionUpdateData[pointerTosubmissionUpdateData].map(async (eachQuestionRow) => {
              eachQuestionRow['questionType'] =
                questionExternalId[eachQuestionRow.questionCode] &&
                questionExternalId[eachQuestionRow.questionCode].responseType != ''
                  ? questionExternalId[eachQuestionRow.questionCode].responseType
                  : 'Question Not Found';
              ...
              if (
                questionExternalId[eachQuestionRow.questionCode] &&
                questionExternalId[eachQuestionRow.questionCode].options &&
                questionExternalId[eachQuestionRow.questionCode].options.length > 0
              ) {
              ...
              if (!questionExternalId[eachQuestionRow.questionCode]) {
                ...
                let ecmByCsv =
                  ...
                  questionExternalId[eachQuestionRow.questionCode].id;
                ...
                let questionValueConversion = await submissionsHelper.questionValueConversion(
                  questionExternalId[eachQuestionRow.questionCode],
                  eachQuestionRow.oldResponse,
                  eachQuestionRow.newResponse,
                );

                if (
                  !questionValueConversion.oldValue ||
                  !questionValueConversion.newValue ||
                  questionValueConversion.oldValue == '' ||
                  questionValueConversion.newValue == ''
                ) {
                  ...
                  let submissionCheck = await database.models.submissions
      ...
      try {
        ...
        let updateQuery = {
          $set: {
            ...
            [evidencesToBeReset + '.endTime']: '',
            [evidencesToBeReset + '.startTime']: '',
            [evidencesToBeReset + '.hasConflicts']: false,
            'evidencesStatus.$.submissions': [],
            'evidencesStatus.$.isSubmitted': false,
            'evidencesStatus.$.hasConflicts': false,
            'evidencesStatus.$.startTime': '',
            'evidencesStatus.$.endTime': '',
          },
        };

        submissionUpdated.push({
          userId: req.userDetails.id,
          date: new Date(),
          message: 'Updated ECM ' + req.query.ecm,
        });
  ...
  async delete(req) {
    return new Promise(async (resolve, reject) => {
      try {
        let submissionData = await submissionsHelper.delete(req.params._id, req.userDetails.userId);
  ...
  async setTitle(req) {
    return new Promise(async (resolve, reject) => {
      try {
        let editedSubmissionTitle = await submissionsHelper.setTitle(
          req.params._id,
          req.body.title,
  ...
  */
  ...
  async create(req) {
    return new Promise(async (resolve, reject) => {
      try {
        let response = await submissionsHelper.create(
          req.params._id,
          req.query.entityId,
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (submissionCriteria.data && submissionCriteria.data.length > 0) {
          for (
            let pointerToSubmissionCriteria = 0;
            pointerToSubmissionCriteria < submissionCriteria.data.length;
            pointerToSubmissionCriteria++
          ) {
            const criteria = submissionCriteria.data[pointerToSubmissionCriteria];
            result.push(_.pick(criteria, ['name', 'description', 'externalId', 'score', 'improvement-projects']));
          }
        }
  ...
  */
  ...
  async list(req) {
    return new Promise(async (resolve, reject) => {
      try {
        let submissionDocument = await submissionsHelper.list(req.query.entityId, req.params._id);
  ...
  */
  ...
  async getCriteriaQuestions(req) {
    return new Promise(async (resolve, reject) => {
      try {
        let criteriaQuestions = await submissionsHelper.getCriteriaQuestions(req.params._id);
  ...
  */
  ...
  async manualRating(req) {
    return new Promise(async (resolve, reject) => {
      try {
        let response = await submissionsHelper.manualRating(req.params._id, req.body, req.userDetails.userId);
