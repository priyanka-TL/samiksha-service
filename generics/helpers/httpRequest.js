...
var Request = class Request {
      ...
      req = httpModule.request(options, function (res) {
        ...
        res.on('end', function (content) {
          resolve({
            data: responseData,
            message: 'Success',
            status: res.status ? res.status : res.statusCode ? res.statusCode : '',
            headers: res.headers,
          });
        });
      });
  ...
  _request(requestUrl, options, data, path) {
    ...
    if (!path) return this._httpRequest(options, data);
    else return this._httpFileRequest(options, data, path);
  }
