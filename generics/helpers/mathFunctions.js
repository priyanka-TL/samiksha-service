...
math.import({
  compareTextValues: function (string1, string2) {
    ...
    if (matchOperator === 'or') {
      for (
        let pointerToSearchUniverseArray = 0;
        pointerToSearchUniverseArray < searchUniverse.length;
        pointerToSearchUniverseArray++
      ) {
  ...
  checkIfPresent: function (needle, haystack) {
    let searchUniverse = new Array();
    if (haystack._data) {
      searchUniverse = haystack._data;
    } else {
      searchUniverse = haystack;
    }
    ...
    if (matchOperator === 'or') {
      for (
        let pointerToSearchParametersArray = 0;
        pointerToSearchParametersArray < searchParameters.length;
        pointerToSearchParametersArray++
      ) {
        if (
          searchUniverse.findIndex(
            (arrayElement) => arrayElement === searchParameters[pointerToSearchParametersArray],
          ) >= 0
        ) {
  ...
  checkIfModeIs: function (needle, haystack, optionRank = false) {
    ...
    if (needle._data) {
      ...
      let countOfElements = Object.entries(_.countBy(haystack)).sort((a, b) => {
        return b[1] - a[1];
      });

      isMode =
        _.isEqual(countOfElements[0][0].split(','), searchKey) &&
        (countOfElements[1] ? countOfElements[0][1] > countOfElements[1][1] : true)
          ? 1
          : -1;
    } else {
      if (typeof needle != 'string' && typeof needle != 'number') {
      ...
      if (countOfElements[0] && countOfElements[1] && countOfElements[0][1] == countOfElements[1][1] && optionRank) {
        ...
        for (
          let pointerToElementsWithHighestCount = 0;
          pointerToElementsWithHighestCount < elementsWithHighestCount.length;
          pointerToElementsWithHighestCount++
        ) {
          if (!modeWinnerFound) {
            ...
            optionRankFromLowToHigh.indexOf(elementsWithHighestCount[pointerToElementsWithHighestCount]) < modeWinner
          ) {
            modeWinner = optionRankFromLowToHigh.indexOf(elementsWithHighestCount[pointerToElementsWithHighestCount]);
          }
        }
        if (modeWinnerFound) {
          ...
          countOfElements = Object.entries(_.countBy(haystack)).sort((a, b) => {
            return b[1] - a[1];
          });
        }
      }
      ...
      if (matchOperator === 'or') {
        for (
          ...
          modeValueCalculated =
            countOfElements[0][0] === searchParameters[pointerToSearchParametersArray] &&
            (countOfElements[1] ? countOfElements[0][1] > countOfElements[1][1] : true)
              ? 1
              : -1;
  ...
  modeValue: function (haystack) {
    const countOfElements = Object.entries(_.countBy(haystack)).sort((a, b) => {
      return b[1] - a[1];
    });
