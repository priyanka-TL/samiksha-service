...
const sendKafkaErrorMessagesToSlack =
  !process.env.KAFKA_ERROR_MESSAGES_TO_SLACK || !process.env.KAFKA_ERROR_MESSAGES_TO_SLACK != 'OFF' ? 'ON' : 'OFF';
...
const sendExceptionLogMessage = function (errorObject) {
  if (slackCommunicationsOnOff === 'ON' && slackToken != '' && gen.utils.checkIfStringIsUrl(exceptionLogPostUrl)) {
    ...
    Object.keys(_.pick(errorObject.headers, ['x-channel-id', 'gpslocation', 'x-authenticated-userid'])).forEach(
      (objProperty) => {
        fieldsData.push({
          title: objProperty,
    ...
    new Promise((resolve, reject) => {
      return resolve(reqObj.post(exceptionLogPostUrl, options));
    })
      .then((result) => {
        if (result.data === 'ok') {
          returnResponse = {
...
const rubricErrorLogs = function (errorMessage) {
  if (
    slackCommunicationsOnOff === 'ON' &&
    sendRubricErrorMessagesToSlack === 'ON' &&
    slackToken != '' &&
    gen.utils.checkIfStringIsUrl(exceptionLogPostUrl)
  ) {
    const reqObj = new Request();
    let attachmentData = new Array();
    let fieldsData = new Array();
...
const badSharedLinkAccessAttemptAlert = function (errorMessage) {
  if (slackCommunicationsOnOff === 'ON' && slackToken != '' && gen.utils.checkIfStringIsUrl(exceptionLogPostUrl)) {
    ...
    Object.keys(_.pick(errorMessage.headers, ['x-channel-id', 'gpslocation', 'x-authenticated-userid'])).forEach(
      (objProperty) => {
        fieldsData.push({
          title: objProperty,
    ...
    errorMessage.customFields &&
      Object.keys(errorMessage.customFields).forEach((customField) => {
        fieldsData.push({
          title: customField,
