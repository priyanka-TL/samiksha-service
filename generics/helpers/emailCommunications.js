const emailCommunicationsOnOff =
  !process.env.EMAIL_COMMUNICATIONS_ON_OFF || process.env.EMAIL_COMMUNICATIONS_ON_OFF != 'OFF' ? 'ON' : 'OFF';
const emailServiceBaseUrl =
  process.env.EMAIL_SERVICE_BASE_URL && process.env.EMAIL_SERVICE_BASE_URL != ''
    ? process.env.EMAIL_SERVICE_BASE_URL
    : '';
const emailServiceToken =
  process.env.EMAIL_SERVICE_TOKEN && process.env.EMAIL_SERVICE_TOKEN != '' ? process.env.EMAIL_SERVICE_TOKEN : '';
...
const pushMailToEmailService = function (recipients = '', subject = '', text = '') {
  return new Promise(async (resolve, reject) => {
    try {
      if (emailCommunicationsOnOff != 'ON' || emailServiceBaseUrl == '' || emailServiceToken == '')
      ...
      let response = await reqObj.post(emailServiceBaseUrl, options);
