...
const fs = require('fs');
const accessTokenValidationMode =
  process.env.VALIDATE_ACCESS_TOKEN_OFFLINE && process.env.VALIDATE_ACCESS_TOKEN_OFFLINE === 'OFF' ? 'OFF' : 'ON';
const keyCloakPublicKeyPath =
  process.env.KEYCLOAK_PUBLIC_KEY_PATH && process.env.KEYCLOAK_PUBLIC_KEY_PATH != ''
    ? ROOT_PATH + '/' + process.env.KEYCLOAK_PUBLIC_KEY_PATH + '/'
    : ROOT_PATH + '/' + 'keycloak-public-keys/';
