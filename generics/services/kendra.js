...
const getUsersByEntityAndRole = function (entityId = '', role = '') {
  return new Promise(async (resolve, reject) => {
    try {
      const url =
        kendraServiceBaseURL +
        messageConstants.endpoints.GET_USERS_BY_ENTITY_AND_ROLE +
        '/' +
        entityId +
        '?role=' +
        role;
...
const solutionBasedOnRoleAndLocation = function (token, bodyData, type, searchText = '') {
  return new Promise(async (resolve, reject) => {
    try {
      const url =
        kendraServiceBaseURL +
        messageConstants.endpoints.SOLUTIONS_BASED_ON_ROLE_LOCATION +
        '?type=' +
        type +
        '&search=' +
        searchText;
