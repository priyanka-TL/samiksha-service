module.exports = {
  async up(db) {
    ...
    if (process.env.TRANSFER_FROM_DB !== '') {
      ...
      await Promise.all(
        evaluationFrameworks.map(async (eachFrameworkDocument) => {
          ...
          let frameworkCriteria = await db
            .collection('criteria')
            .find({ _id: { $in: frameworkCriteriaArray } })
            .toArray();
          ...
          eachFrameworkDocument.updatedBy = eachFrameworkDocument.updatedBy
            ? eachFrameworkDocument.updatedBy
            : 'INITIALIZE';
          eachFrameworkDocument.createdBy = eachFrameworkDocument.updatedBy
            ? eachFrameworkDocument.updatedBy
            : 'INITIALIZE';
          eachFrameworkDocument.createdAt = eachFrameworkDocument.createdAt
            ? eachFrameworkDocument.createdAt
            : new Date();
          eachFrameworkDocument.updatedAt = eachFrameworkDocument.updatedAt
            ? eachFrameworkDocument.updatedAt
            : new Date();
      ...
      global.migrationMsg =
        'Total evaluationFrameworks transferred - ' +
        evaluationFrameworks.length +
        ' and total criteria transferred - ' +
        allCriteria.length;
