module.exports = {
  async up(db) {
    ...
    let userExtensions = await db.collection('userExtension').find({}).project({ _id: 1 }).toArray();
    ...
    for (let users = 0; users < chunkOfUsers.length; users++) {
      ...
      let userDocumentArray = await db
        .collection('userExtension')
        .find({
          _id: { $in: usersId },
        })
        .project({
      ...
      await Promise.all(
        userDocumentArray.map(async (userDocument) => {
          await es.update({
