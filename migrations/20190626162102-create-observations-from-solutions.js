module.exports = {
  async up(db) {
    ...
    if (solutionDocuments.length > 0) {
      ...
      await Promise.all(
        solutionDocuments.map(async (solutionDocument) => {
          let entityAssessorDocuments = await db
            .collection('entityAssessors')
            .find({
          ...
          let parentSolutionDocument = await db
            .collection('solutions')
            .find({
          ...
          await Promise.all(
            entityAssessorDocuments.map(async (entityAssessor) => {
              ...
              if (observationId.insertedId) {
                ...
                for (
                  ...
                  oldSubmissionDocumentsArray = await db
                    .collection('submissions')
                    .find({
                      _id: {
                  ...
                  await Promise.all(
                    oldSubmissionDocumentsArray.map(async (eachSubmissionDocument) => {
                      try {
                        ...
                        let observationSubmissionId = await db
