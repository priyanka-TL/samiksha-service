  ...
  async up(db) {
    ...
    if (process.env.TRANSFER_FROM_DB !== '') {
      ...
      for (let schoolAssessorCounter = 0; schoolAssessorCounter < schoolAssessors.length; schoolAssessorCounter++) {
        if (!schoolAssessors[schoolAssessorCounter].userId || schoolAssessors[schoolAssessorCounter].userId == '')
          continue;
        ...
        schoolAssessorsArray.push(_.omit(schoolAssessors[schoolAssessorCounter], ['schools']));
      }
