module.exports = {
  async up(db) {
    ...
    let solutionDocuments = await db
      ...
      .find({ 'entityProfileFieldsPerEntityTypes.A1': { $exists: true } })
      .project({ roles: 1, 'entityProfileFieldsPerEntityTypes.A1': 1 })
      .toArray();

    if (solutionDocuments.length > 0) {
      global.migrationMsg =
        'Change schoolTypes to types in solution entity profile fields and in entityTypes collection.';
      ...
      let entityTypeDocuments = await db
        .collection('entityTypes')
        .find({ profileFields: { $exists: true } })
        .project({ profileFields: 1 })
        .toArray();

      await Promise.all(
        entityTypeDocuments.map(async (entityType) => {
