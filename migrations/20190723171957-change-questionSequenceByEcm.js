module.exports = {
  async up(db) {
    let solutionDocuments = await db
      ...
      .find({ questionSequenceByEcm: { $exists: true } })
      .project({ questionSequenceByEcm: 1, sections: 1 })
      .toArray();
          ...
          if (solution.questionSequenceByEcm && solution.sections) {
            ...
            Object.keys(solution.questionSequenceByEcm).forEach((ecmCode) => {
              Object.keys(solution.questionSequenceByEcm[ecmCode]).forEach((sectionName) => {
                ...
                if (!newquestionSequenceByEcm[ecmCode][sectionNameToCodeMap[sectionName]])
                  newquestionSequenceByEcm[ecmCode][sectionNameToCodeMap[sectionName]] =
                    solution.questionSequenceByEcm[ecmCode][sectionName];
              });
            });
