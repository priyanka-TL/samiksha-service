module.exports = {
  async up(db) {
    ...
    if (process.env.TRANSFER_FROM_DB !== '') {
      ...
      for (
        let individualEntityCounter = 0;
        individualEntityCounter < individualEntities.length;
        individualEntityCounter++
      ) {
        individualEntitiesArray.push({
          ...
          updatedBy: individualEntities[individualEntityCounter].updatedBy
            ? individualEntities[individualEntityCounter].updatedBy
            : 'INITIALIZE',
          createdBy: individualEntities[individualEntityCounter].createdBy
            ? individualEntities[individualEntityCounter].createdBy
            : 'INITIALIZE',
          createdAt: individualEntities[individualEntityCounter].createdAt
            ? individualEntities[individualEntityCounter].createdAt
            : new Date(),
          updatedAt: individualEntities[individualEntityCounter].updatedAt
            ? individualEntities[individualEntityCounter].updatedAt
            : new Date(),
          isDeleted: false,
        });
      }
