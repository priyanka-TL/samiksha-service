module.exports = {
  async up(db) {
    ...
    if (process.env.TRANSFER_FROM_DB !== '') {
      ...
      for (
        let pointerToInsightsIdDocument = 0;
        pointerToInsightsIdDocument < chunkOfInsightIdsDocument.length;
        pointerToInsightsIdDocument++
      ) {
        ...
        oldInsightsDocumentsArray = await sourceDB
          .collection('insights')
          .find({
            _id: {
              $in: fetchInsightsIds,
        ...
        await Promise.all(
          oldInsightsDocumentsArray.map(async (eachInsightDocument) => {
            eachInsightDocument.isDeleted = false;
            eachInsightDocument.entityId = eachInsightDocument.entityId
              ? eachInsightDocument.entityId
              : eachInsightDocument.schoolId;
            eachInsightDocument.entityExternalId = eachInsightDocument.entityExternalId
              ? eachInsightDocument.entityExternalId
              : eachInsightDocument.schoolExternalId;
            ...
            eachInsightDocument.entityName = eachInsightDocument.entityName
              ? eachInsightDocument.entityName
              : eachInsightDocument.schoolName;

            eachInsightDocument.entityTypeId =
              solutionDocumentMap[eachInsightDocument.evaluationFrameworkId.toString()].entityTypeId;
            eachInsightDocument.entityType =
              solutionDocumentMap[eachInsightDocument.evaluationFrameworkId.toString()].entityType;

            await db
              .collection('insights')
              .insertOne(
                _.omit(eachInsightDocument, [
                  'schoolName',
                  'schoolExternalId',
                  'schoolId',
