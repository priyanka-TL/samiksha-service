module.exports = {
  async up(db) {
    ...
    let criteriaDocuments = await db
      .collection('criteria')
      .find({ 'rubric.levels.L1.expression': { $eq: '' } })
      .project({ rubric: 1, criteriaType: 1 })
      .toArray();

    if (criteriaDocuments.length > 0) {
      global.migrationMsg =
        'Set criteriaType to manual for non algo criteria and update solutions/frameworks accordingly';
      ...
      let frameworkAndSolutionTypes = ['observation', 'assessment'];

      let frameworkDocuments = await db
        .collection('frameworks')
        .find({ isRubricDriven: true })
        .project({ themes: 1, isRubricDriven: 1 })
        .toArray();

      await Promise.all(
        frameworkDocuments.map(async (framework) => {
          if (framework.themes.length > 0) {
            ...
            let criteriaDocuments = await db
              .collection('criteria')
              .find({ _id: { $in: allFrameworkCriteriaIds }, criteriaType: 'auto' })
              .project({ _id: 1 })
              .toArray();
      ...
      let solutionDocuments = await db
        ...
        .find({ isRubricDriven: true, type: { $in: frameworkAndSolutionTypes } })
        .project({ themes: 1, isRubricDriven: 1 })
        .toArray();
      ...
      await Promise.all(
        solutionDocuments.map(async (solution) => {
          if (solution.themes.length > 0) {
            ...
            let criteriaDocuments = await db
              .collection('criteria')
              .find({ _id: { $in: allSolutionCriteriaIds }, criteriaType: 'auto' })
              .project({ _id: 1 })
              .toArray();
