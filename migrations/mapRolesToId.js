...
async function getSubRolesData(professional_role_id){
    ...
    var options = {
      'method': 'GET',
      'url': `https://saas-qa.tekdinext.com/entity-management/v1/entities/subEntityList/${professional_role_id}?type=professional_subroles`,
      'headers': {
...
async function getProfessionalRolesData(){
    ...
    var options = {
        'method': 'GET',
        'url': 'https://saas-qa.tekdinext.com/entity-management/v1/entities/entityListBasedOnEntityType?entityType=professional_role',
        'headers': {
          'Content-Type': 'application/json',
          'tenantId': 'shikshagraha'
        }
...
async function modifyCollection(collectionName) {
	...
	const db = dbClient.db(process.env.DB); // default DB from connection string
	...
	while (await cursor.hasNext()) {
...

        {
            ...
            if(updatedScope.hasOwnProperty('professional_role') && updatedScope['professional_role'].length >= 1 && 
        ...
        if(updatedScope.hasOwnProperty('professional_subroles') && updatedScope['professional_subroles'].length >= 1 &&
