module.exports = {
  async up(db) {
    ...
    if (allEntityTypes.length > 0) {
      ...
      for (
        let pointerToAllEntityTypes = 0;
        pointerToAllEntityTypes < allEntityTypes.length;
        pointerToAllEntityTypes++
      ) {
        await db
          .collection('submissions')
          .updateMany(
            { entityTypeId: allEntityTypes[pointerToAllEntityTypes]._id },
            { $set: { entityType: allEntityTypes[pointerToAllEntityTypes].name } },
          );
