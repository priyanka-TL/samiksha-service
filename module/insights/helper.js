module.exports = class insightsHelper {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let submissionDocument = await database.models.submissions
          .findOne(submissionsQueryObject, submissionsProjectionObject)
          .lean();
              ...
              theme.criteria.forEach((criteria) => {
                if (criteriaScore[criteria.criteriaId.toString()]) {
                  criteriaScoreArray.push({
                    ...
                    score: levelToScoreMapping[criteriaScore[criteria.criteriaId.toString()].score]
                      ? levelToScoreMapping[criteriaScore[criteria.criteriaId.toString()].score].points
                      : 'NA',
        ...
        let themeAndCriteriaScores = scoreThemes(
          solutionDocument.themes,
          solutionDocument.levelToScoreMapping,
          criteriaScore,
          0,
          [],
        );
  ...
  static insightsDocument(programId, entityIds) {
    return new Promise(async (resolve, reject) => {
      try {
        let insightDocument = await database.models.insights
          .find(
            {
