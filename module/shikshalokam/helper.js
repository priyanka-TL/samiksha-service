...
const userProfileFetchEndpoint =
  process.env.SHIKSHALOKAM_USER_PROFILE_FETCH_ENDPOINT && process.env.SHIKSHALOKAM_USER_PROFILE_FETCH_ENDPOINT != ''
    ? process.env.SHIKSHALOKAM_USER_PROFILE_FETCH_ENDPOINT
    : '/api/user/v1/read';
const shikshalokamBaseHost =
  process.env.SHIKSHALOKAM_BASE_HOST && process.env.SHIKSHALOKAM_BASE_HOST != ''
    ? process.env.SHIKSHALOKAM_BASE_HOST
    : '';
...
module.exports = class ShikshalokamHelper {
  static getUserOrganisation(authToken = '', keycloakUserId = '') {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (
          userOrganisationDetails.success &&
  ...
  static getUserDetails(authToken = '', keycloakUserId = '') {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let userProfileResponse = await reqObj.get(requestURL, {
  ...
  static getOrganisationsAndRootOrganisations(token, userId) {
    return new Promise(async (resolve, reject) => {
      try {
        let userOrganisations = await this.getUserOrganisation(token, userId);
