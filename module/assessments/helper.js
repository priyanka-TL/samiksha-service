...
module.exports = class AssessmentsHelper {
    ...
    return new Promise(async (resolve, reject) => {
      try {
            ...
            section.questions.forEach((question, index, section) => {
              ...
              if (_.difference(question.questionGroup, questionGroup).length < question.questionGroup.length) {
                ...
                if (question.prefillFromEntityProfile) {
                  question['value'] = entityMetaInformation[question.entityFieldName]
        ...
        Object.entries(questionArray).forEach((questionArrayElm) => {
          ...
          if (!questionArrayElm[1].file || !Object.keys(questionArrayElm[1].file).length)
            questionArrayElm[1]['file'] = '';
          ...
          if (questionArrayElm[1].responseType === 'matrix') {
            ...
            questionArrayElm[1].instanceQuestions.forEach((instanceQuestionId) => {
              ...
              if (
                ...
                submissionsObjects[questionArrayElm[1].evidenceMethod].answers[questionArrayElm[0]].value.forEach(
                  (singleValue) => {
                    if (
                      singleValue[instanceQuestionId.toString()].fileName &&
        ...
        if (questionSequenceByEcm) {
          evidences.forEach((evidence) => {
            if (questionSequenceByEcm[evidence.externalId]) {
              evidence.sections.forEach((section) => {
                if (
                  questionSequenceByEcm[evidence.externalId][section.code] &&
                  questionSequenceByEcm[evidence.externalId][section.code].length > 0
                ) {
                ...
                submissionsObjects[questionArrayElm[1].evidenceMethod].answers[questionArrayElm[0]].value.forEach(
                  (singleValue) => {
                    questionArrayElm[1].instanceQuestions.forEach((instanceQuestion) => {
                      ...
                      if (
                        singleValue[instanceQuestionId].fileName &&
                        singleValue[instanceQuestionId].fileName.length > 0
                      ) {
          ...
          if (parseQuestionV1.pageQuestionsEnabled[eachEvidence.externalId]) {
            eachEvidence.sections.forEach((eachSection) => {
              if (parseQuestionV1.pageQuestionsEnabled[eachEvidence.externalId][eachSection.code]) {
                ...
                for (
                  let pointerToEachSectionQuestion = 0;
                  pointerToEachSectionQuestion < eachSection.questions.length;
                    ...
                    if (!pageQuestionsObj[pageName]) {
                      if (!(Object.keys(defaultQuestion).length > 0)) {
                        Object.keys(eachQuestion).forEach((questionModelKey) => {
                          if (
                            questionModelKey === 'updatedAt' ||
                            questionModelKey === 'createdAt' ||
                            questionModelKey === '_id'
                          ) {
  ...
  static programDocumentFieldListInResponse() {
    return new Promise(async (resolve, reject) => {
      return resolve(['_id', 'externalId', 'name', 'description', 'imageCompression', 'isAPrivateProgram']);
    });
  }
  ...
  static metaForm(solutionId) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let filteredData = {
          name:
            solutionsData[0].assessmentMetaFormKey && solutionsData[0].assessmentMetaFormKey !== ''
              ? solutionsData[0].assessmentMetaFormKey
              : defaultForm,
        };

        let assessmentMetaForm = await formsHelper.formDocuments(filteredData, ['value']);
