...
module.exports = class IndividualAssessmentHelper {
  ...
  static list(search, limit, pageNo, userId, token) {
    return new Promise(async (resolve, reject) => {
      try {
        let individualAssessmentsSolution = await solutionsHelper.templates(
  ...
  static details(templateId) {
    return new Promise(async (resolve, reject) => {
      try {
        let individualAssessmentSolution = await assessmentsHelper.templateDetails(templateId, true, true);
        ...
        return resolve({
          message: messageConstants.apiResponses.INDIVIDUAL_SOLUTIONS_DETAILS,
