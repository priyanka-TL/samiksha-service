...
module.exports = class PollsHelper {
  ...
  static pollDocuments(pollFilter = 'all', fieldsArray = 'all', sortedData = '', skipFields = 'none') {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (sortedData !== '') {
          pollDocuments = await database.models.polls.find(queryObject, fieldsArray).sort(sortedData).lean();
        } else {
          pollDocuments = await database.models.polls.find(queryObject, projection).lean();
        }
    ...
    return new Promise(async (resolve, reject) => {
      try {
        let pollCreationForm = await formsHelper.formDocuments(
          {
            name: {
              $in: [messageConstants.common.POLL_METAFORM, messageConstants.common.POLL_QUESTION_METAFORM],
            },
          },
          ['value'],
        );

        pollCreationForm = [...pollCreationForm[0].value, ...pollCreationForm[1].value];
  ...
  static report(pollId = '') {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let pollDocument = await this.pollDocuments({ _id: pollId }, ['result', 'questions', 'numberOfResponses']);
  ...
  static updatePollDocument(query = {}, updateObject = {}) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let updateResponse = await database.models.polls.updateOne(query, updateObject);
  ...
  static getPollLinkById(pollId = '', appName) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let pollLink = await this.pollDocuments({ _id: pollId, endDate: { $gt: new Date() } }, ['link']);
