...
module.exports = class SubmissionsHelper {
  ...
  static findSubmissionByEntityProgram(document, userAgent, userId) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let submissionDocument = await this.submissionDocuments(queryObject, ['assessors']);

        if (!submissionDocument[0]) {
          document.assessors = await this.assessors(document.solutionId, document.entityId, userAgent, userId);

          submissionDocument = await database.models.submissions.create(document);
          ...
          if (assessorElement && assessorElement.externalId != '') {
            ...
            submissionDocument = await database.models.submissions.findOneAndUpdate(queryObject, updateObject);
          }
        }
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (question.responseType == 'date') {
          ...
          result['oldValue'] = oldResponseArray
            .map((value) => (value < 10 ? '0' + value : value))
            .reverse()
            .join('-');
          result['newValue'] = newResponseArray
            .map((value) => (value < 10 ? '0' + value : value))
            .reverse()
            .join('-');
          ...
          let oldResponseArray = oldResponse.split(',').map((value) => {
            return value.replace(/\s/g, '').toLowerCase();
          });
          let newResponseArray = newResponse.split(',').map((value) => {
            return value.replace(/\s/g, '').toLowerCase();
          });
      ...
      try {
        ...
        if (req.body.evidence) {
          ...
          let evidencesStatusToBeChanged = submissionDocument.evidencesStatus.find(
            (singleEvidenceStatus) => singleEvidenceStatus.externalId == req.body.evidence.externalId,
          );
          ...
          if (submissionDocument.evidences[req.body.evidence.externalId].isSubmitted === false) {
            ...
            let submissionAllowed = await this.isAllowed(req.params._id, req.body.evidence.externalId, modelName);
  ...
  static getAnswersFromGeneralQuestion(answer, submissionDocument) {
    if (
      ...
      submissionDocument.generalQuestions[answer[0]]
    ) {
  ...
  static pushSubmissionToQueueForRating(submissionId = '') {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const kafkaMessage = await kafkaClient.pushSubmissionToKafkaQueueForRating({
    ...
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        if (resultingArray.result.runUpdateQuery) {
          ...
          emailClient.pushMailToEmailService(
            emailRecipients,
            messageConstants.apiResponses.SUBMISSION_AUTO_RATING_FAILED + ' - ' + submissionId,
            JSON.stringify(resultingArray),
          );
          return resolve(messageConstants.apiResponses.SUBMISSION_RATING_COMPLETED);
        }
  ...
  static delete(submissionId, userId) {
    return new Promise(async (resolve, reject) => {
      try {
        let submissionDocument = await database.models.submissions.deleteOne({
          _id: submissionId,
  ...
  static setTitle(submissionId, updatedTitle, userId) {
    return new Promise(async (resolve, reject) => {
      try {
        let submissionDocument = await database.models.submissions.findOneAndUpdate(
          {
            _id: submissionId,
            'assessors.userId': userId,
          },
  ...
  static create(solutionId, entityId, userAgent, userId) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let submissionDocumentEvidences = this.evidences(solutionDocument[0].evidenceMethods);

        let submissionDocumentCriterias = await this.criterias(solutionDocument[0].themes);

        let submissionData = {
          entityId: entityDocument[0]._id,
          entityExternalId: entityDocument[0].metaInformation.externalId
  ...
  static createASubmission(submissionData, userAgent, userId) {
    return new Promise(async (resolve, reject) => {
      try {
        const lastSubmission = await this.findLastSubmission(submissionData.solutionId, submissionData.entityId);

        submissionData.submissionNumber = lastSubmission + 1;
        submissionData.assessors = await this.assessors(
        ...
        let submissionDocument = await database.models.submissions.create(submissionData);
  ...
  static criterias(themes) {
    return new Promise(async (resolve, reject) => {
      try {
        let criteriaIdArray = gen.utils.getCriteriaIdsAndWeightage(themes);
        ...
        let criteriaQuestionDocument = await criteriaQuestionsHelper.list({ _id: { $in: criteriaId } }, 'all', [
          'resourceType',
          'language',
          'keywords',
          'concepts',
        ...
        let submissionDocumentCriterias = criteriaQuestionDocument.map((criteria) => {
  ...
  static assessors(solutionId, entityId, userAgent, userId) {
    return new Promise(async (resolve, reject) => {
      try {
        let assessorDocument = await entityAssessorsHelper.assessorsDocument({
          solutionId: solutionId,
          entities: entityId,
        });

        let assessorElement = assessorDocument.find((assessor) => assessor.userId === userId);
  ...
  static findLastSubmission(solutionId, entityId) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        return resolve(
          submissionDocument[0] && submissionDocument[0].submissionNumber ? submissionDocument[0].submissionNumber : 0,
        );
      } catch (error) {
        reject(error);
      }
  ...
  static list(entityId, solutionId) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let result = await this.submissionDocuments(queryObject, projection, 'none', {
          createdAt: -1,
        });
  ...
  static getCriteriaQuestions(submissionId = '') {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let projection = ['answers', 'criteria._id', 'criteria.name', 'solutionId'];

        let submissionDocument = await this.submissionDocuments(queryObject, projection);
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (submissionDocument[0]['criteria'] && submissionDocument[0].criteria.length > 0) {
          ...
          for (
            let pointerToSubmissionCriteriaArray = 0;
            pointerToSubmissionCriteriaArray < submissionDocument[0].criteria.length;
            pointerToSubmissionCriteriaArray++
          ) {
            const criteria = submissionDocument[0].criteria[pointerToSubmissionCriteriaArray];
            if (criteriaObject[criteria._id.toString()]) {
              submissionUpdateObject[`criteria.${pointerToSubmissionCriteriaArray}.score`] =
                criteriaObject[criteria._id.toString()];
            }
          }
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (modelName === messageConstants.common.OBSERVATION_SUBMISSIONS) {
          ...
          submissionDocument = await this.submissionDocuments(queryObject, projection);
        }
