...
module.exports = class ProgramOperationsHelper {
  ...
  static getEntities(userDetails, queryParams, pageSize, pageNo, pagination = false, assessorIds, solutionId) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        filteredEntityDocument = database.models.entities
          .find(entitisQueryObject, { _id: 1, 'metaInformation.name': 1, 'metaInformation.externalId': 1 })
          .limit(limitValue)
          .skip(skipValue)
          .lean()
          .exec();
  ...
  static getQueryObject(requestQuery) {
    ...
    let filteredQueries = _.pullAll(queries, [
      'csv',
      'fromDate',
      'toDate',
      'assessorName',
    ...
    filteredQueries.forEach((query) => {
      if (query == 'area') {
        queryObject['$or'] = [
          { 'metaInformation.zoneId': new RegExp(requestQuery.area, 'i') },
          { 'metaInformation.districtName': new RegExp(requestQuery.area, 'i') },
        ];
  ...
  static checkUserAuthorization(userDetails, solutionId) {
    ...
    if (userDetails.accessiblePrograms.length) {
      let userProgramExternalIds = userDetails.accessiblePrograms.find(
