...
module.exports = class EntityAssessorHelper {
  ...
  static assessorsDocument(findQuery = 'all', fields = 'all', skipFields = 'none') {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let assessorsData = await database.models.entityAssessors.find(queryObject, projection).lean();
      ...
      try {
        ...
        assessorData.forEach((assessor) => {
          assessor.entities.split(',').forEach((entityAssessor) => {
            if (entityAssessor) entityIds.push(entityAssessor.trim());
          });
        ...
        let programsFromDatabase = await database.models.programs
          .find(
            {
        ...
        let solutionsFromDatabase = await database.models.solutions
          .find(
            {
              externalId: { $in: solutionIds },
            },
            { externalId: 1, entityType: 1, entityTypeId: 1, entities: 1, name: 1, type: 1, subType: 1 },
          )
          .lean();
        ...
        assessorData = await Promise.all(
          assessorData.map(async (assessor) => {
            ...
            await database.models.entityAssessors.findOneAndUpdate(
              { userId: assessor.userId, programId: assessor.programId, solutionId: fieldsWithOutEntity['solutionId'] },
              updateObject,
              {
                upsert: true,
                new: true,
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let result = await Promise.all(
          userExternalIds.map((userExternalId) => {
  ...
  static sendUserNotifications(userId = '', entities = []) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const kafakResponses = await Promise.all(
  ...
  static pendingAssessment() {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let submissions = await database.models.submissions
          .find(queryData, {
            _id: 1,
          })
          .lean();
        ...
        let chunkOfSubmissionDocuments = _.chunk(submissions, chunkOfSubmissionsLength);
        ...
        for (
          ...
          submissionsIds = chunkOfSubmissionDocuments[pointerToSubmissions].map((submission) => {
          ...
          await Promise.all(
            submissionDocuments.map(async (submissionDocument) => {
              if (submissionDocument.assessors && submissionDocument.assessors.length > 0) {
                let entityName = '';

                if (submissionDocument.entityInformation && submissionDocument.entityInformation.name) {
                  entityName = submissionDocument.entityInformation.name;
                } else if (submissionDocument.entityInformation && submissionDocument.entityInformation.externalId) {
                  entityName = submissionDocument.entityInformation.externalId;
                }
                ...
                submissionDocument.assessors.forEach((assessor) => {
                  if (assessor.role && gen.utils.fetchAssessorsLeadAssessorRole().includes(assessor.role)) {
  ...
  static completedAssessment(fromDate, toDate) {
    return new Promise(async (resolve, reject) => {
      try {
        let queryData = {
  ...
  static create(userDetails, programId, solutionId, entities = []) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let entityAssessor = await this.update(programId, solutionId, userDetails.userId, updateData);
  ...
  static update(programId, solutionId, userId, updateData) {
    return new Promise(async (resolve, reject) => {
      try {
        let assessorData = await database.models.entityAssessors.findOneAndUpdate(
          {
            userId: userId,
            programId: programId,
            solutionId: solutionId,
  ...
  static bulkCreateByUserRoleAndEntity(userAssessmentData, userId) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        await this.upload(assessorData, null, null, userId, null);
